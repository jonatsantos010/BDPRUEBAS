...
{
    public class CtaXcobrarDA : ConnectionBase
    {
        public List<CuentaXcobrarVM> GetBranch()
        {
            var parameters = new List<OracleParameter>();
            var suggests = new List<CuentaXcobrarVM>();
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        var suggest = new CuentaXcobrarVM();
        ...
        public int InsertReportStatus(CtaXcobrarFilter data)
        {
            try
            {
                ResponseControl generic = new ResponseControl(Response.Ok);

                var invoices = new List<CtaXcobrarBM>();
                var parameters = new List<OracleParameter>();
                ...
                this.ExecuteByStoredProcedureVT(string.Format("{0}.{1}", ProcedureName.pkg_ReportesBrokerSCTR, procedure), parameters);
        ...
        {
            var validations = new List<CtaXcobrarBM>();
            ...
            try
            {
                ...
                var reader = (OracleDataReader)this.ExecuteByStoredProcedureVT(string.Format("{0}.{1}", ProcedureName.pkg_ReportesBrokerSCTR, procedure), parameters);

                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        var validation = new CtaXcobrarBM();
